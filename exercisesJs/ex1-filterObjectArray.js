 ...
 * Haz una función que devuelva una lista con todas las ovejas que sean de
 * color rojo y que además su nombre contenga tanto las letras n Y a,
...
export default function contarOvejas(ovejas) {
  const regExp = new RegExp("(?=.*a)(?=.*n)", "gi");
  ovejas = ovejas.filter(
    ({ name, color }) => regExp.test(name) && color === "rojo"
  );
  return ovejas;
}
