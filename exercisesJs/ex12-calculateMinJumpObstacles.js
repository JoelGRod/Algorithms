/**
 * En el taller de Santa 🎅 se están preparando los trineos de
 * motor eléctrico para poder hacer la ruta perfecta para dejar los regalos.
 *
 ...
 *
 * Encima, el trineo sólo se puede configurar para saltar un número
 * fijo de posiciones... 😱
 *
 * Necesitamos una función que nos diga la longitud mínima del salto
 * del trineo para ir evitando todos los obstáculos en la ruta.
 *
...
export default function getMinJump(obstacles) {
  let jump = 1;
  let long = Math.max(...obstacles);
  for (let path = 0; path <= long; path += jump) {
    if (obstacles.includes(path)) {
      path = 0;
      jump += 1;
    }
  }
