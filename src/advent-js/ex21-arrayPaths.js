 ...
 *
 * La ruta siempre va de izquierda a derecha (nunca volverá Santa
 * hacia atrás) pero... ¡ten en cuenta que en mitad de la ruta
 * puede tener que recoger regalos cuando ya tiene alguno encima!
 *
 ...
 *
 * Lo difícil, e importante, es que entiendas que Santa Claus va
 * entregando y recogiendo regalos y que a veces eso puede hacer
 * que supere la carga máxima.
 */
...
export default function canCarry(capacity, trip) {
  ...
  for (let i = 0; i < trip.length; i++) {
    const [gifts, start, end] = trip[i];
    if (start < endPath) totalGifts += gifts;
    else totalGifts = gifts;
    endPath = end;
